...
serve(async (req) => {
  ...
  try {
    ...
    try {
      // Buscar configurações (incluindo template padrão do Facebook, email e flags de habilitação)
      const { data: settings } = await supabase
        .from('nina_settings')
        .select('facebook_lead_template, facebook_lead_email_template, facebook_whatsapp_enabled, facebook_email_enabled')
        .single();
      
      // Verificar se WhatsApp está habilitado
      const whatsappEnabled = settings?.facebook_whatsapp_enabled ?? true;
      const emailEnabled = settings?.facebook_email_enabled ?? true;
      ...
      if (irisError || !irisAgent) {
        ...
        if (conversationId && whatsappEnabled) {
          ...
          console.log('[zapier-leadgen-webhook] WhatsApp DISABLED, skipping template send');
        }
      }
      ...
      if (email && emailEnabled && settings?.facebook_lead_email_template) {
        try {
  ...
  <tr>
    <td style="font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.7;">
      ...
      <span style="display: block; margin-top: 10px;">🌐 <a href="https://jacometoseguros.com.br" style="color: #6366f1; text-decoration: none;">jacometoseguros.com.br</a></span>
        ...
        emailSkipped = true;
        console.log('[zapier-leadgen-webhook] Email DISABLED, skipping email send');
      } else {
      ...
      console.log('[zapier-leadgen-webhook] Success:', { contactId, action, conversationId, templateSent, emailSent, whatsappSkipped, emailSkipped });
